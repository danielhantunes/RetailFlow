...
After provisioning, run ingestion via workflow triggers: **Run Postgres RAW Initial Load** (`run_postgres_raw_initial_load.yml`) for one-time bootstrap/history and **Run Postgres RAW Incremental** (`run_postgres_raw_incremental.yml`) for scheduled/manual incremental runs (cron every 15 min).
...
- **run_postgres_raw_incremental.yml:** Scheduled/manual invocation of the deployed Function App in `INGESTION_MODE=incremental` (cron every 15 min).
