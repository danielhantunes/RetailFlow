...
- **Optional – Bastion (terraform/bastion):** **Azure Bastion (Basic)** in `AzureBastionSubnet` for browser SSH to the private bootstrap VM. Run **after** base apply. **Destroy when idle** (e.g. after Olist load) to stop Bastion hourly billing; use **Azure Function** for ongoing Postgres → ADLS. Managed by **[Terraform Bastion (Dev)](.github/workflows/terraform-bastion-dev.yml)** — action: `plan` \| `apply` \| `destroy`. State: `retailflow-dev-bastion.tfstate`. **Before destroying base**, run **Terraform Bastion (Dev)** `destroy` if Bastion exists (subnet is dedicated to Bastion).
...
- **terraform-bastion-dev.yml:** Optional Azure Bastion (Basic). After base. `destroy` when idle to save cost.
