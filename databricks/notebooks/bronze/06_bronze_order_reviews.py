...
# MAGIC %md
# MAGIC # Bronze: Order reviews
# MAGIC - Read RAW JSONL from ADLS (`postgres_ingest/order_reviews` by default).
...
raw_rel = spark.conf.get("retailflow.raw_order_reviews_rel_path", "postgres_ingest/order_reviews")
...
df.write.format("delta").mode("append").option("mergeSchema", "true").partitionBy("ingestion_date").saveAsTable(
    f"{catalog}.bronze.order_reviews"
)
