...
# MAGIC %md
# MAGIC # Bronze: Order payments
# MAGIC - Read RAW JSONL from ADLS (`postgres_ingest/order_payments` by default).
...
raw_rel = spark.conf.get("retailflow.raw_order_payments_rel_path", "postgres_ingest/order_payments")
...
df.write.format("delta").mode("append").option("mergeSchema", "true").partitionBy("ingestion_date").saveAsTable(
    f"{catalog}.bronze.order_payments"
)
