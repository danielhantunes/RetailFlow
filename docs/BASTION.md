# Azure Bastion (Basic) — On-demand Browser SSH
...
2. Terraform **Bastion (Dev)** has been applied (creates `AzureBastionSubnet` + Bastion Basic + public IP).
...
### Case B: VM uses SSH key authentication (key-only)
