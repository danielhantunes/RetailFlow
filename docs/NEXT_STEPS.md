...
- **Function runtime execution:** Use **Run Postgres RAW Initial Load** (`run_postgres_raw_initial_load.yml`) for one-time bootstrap/history and **Run Postgres RAW Incremental** (`run_postgres_raw_incremental.yml`) for scheduled/manual incremental ingestion every 15 minutes.
