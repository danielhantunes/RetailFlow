...
- Optional lifecycle split: use **Terraform ADLS (Dev)** (`terraform-adls-dev.yml`) + `terraform/adls` to manage `retailflowdevdls` in a separate state when you want storage to persist independently from base workflow destroys. Migrate ADLS state ownership from base before applying the ADLS stack.
...
- **Azure Bastion (on-demand):** Run **Terraform Bastion (Dev)** (`terraform-bastion-dev.yml`) after base when you need browser SSH to the bootstrap VM; **destroy** when idle to save cost (e.g. after Olist load). Bastion is fixed to **Standard** (for Entra login flow). Optionally pass `aad_admin_object_id` (workflow grants `Virtual Machine Administrator Login`). Ongoing ingestion: Azure Function.
...
- **Function runtime execution:** Azure schedules recurring ingestion via Function timer app setting `POSTGRES_TIMER_SCHEDULE` (default every 15 minutes). Use **Run Postgres RAW Initial Load** (`run_postgres_raw_initial_load.yml`) for one-time bootstrap/history and **Run Postgres RAW Incremental** (`run_postgres_raw_incremental.yml`) for manual ad-hoc incremental runs.
...
- **Mandatory when PostgreSQL is the source of origin:** If your source data is PostgreSQL (Olist), running **Provision PostgreSQL for Olist** is required before provisioning/running the Postgres ingest function; otherwise there is no source dataset to ingest. Recommended order: **Base → Postgres (`full` or `apply` + `bootstrap_only`) → Postgres Ingest Function**. Bastion remains optional for access/debug only.
