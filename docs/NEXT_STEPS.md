...
- **Target flow:** PostgreSQL → **Azure Function** (timer; manual via **`POST /api/postgres-to-raw/run`**) → ADLS RAW (`postgres_ingest/...`) → Databricks Bronze/Silver → Snowflake Gold → dbt → analytics marts. See [DATA_FLOW.md](DATA_FLOW.md) and [README — Target data flow](../README.md#target-data-flow).
...
- **Function runtime execution:** Recurring runs use **`POSTGRES_TIMER_SCHEDULE`** (default every 15 minutes). **Ad-hoc:** **Run Postgres RAW Initial Load** / **Incremental** set `INGESTION_MODE` and call **`/api/postgres-to-raw/run`** with the host key (see module README).
