...
- **Azure Bastion (on-demand):** Run **Terraform Bastion (Dev)** (`terraform-bastion-dev.yml`) after base when you need browser SSH to the bootstrap VM; **destroy** when idle to save cost (e.g. after Olist load). For Entra login on VM, use `bastion_sku=Standard` and pass `aad_admin_object_id` (workflow grants `Virtual Machine Administrator Login`). Ongoing ingestion: Azure Function.
