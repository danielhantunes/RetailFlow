...
- **Azure Bastion (on-demand):** Run **Terraform Bastion (Dev)** (`terraform-bastion-dev.yml`) after base when you need browser SSH to the bootstrap VM; **destroy** when idle to save cost (e.g. after Olist load). Ongoing ingestion: Azure Function.
