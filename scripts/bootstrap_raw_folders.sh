...
set -e
RAW_ROOT="${RAW_ROOT:-abfss://raw@retailflowdevdls.dfs.core.windows.net/data/raw}"
for entity in orders customers products inventory clickstream payments store_sales; do
  echo "Creating $RAW_ROOT/$entity"
  # az storage fs directory create -n "data/raw/$entity" -f raw --account-name retailflowdevdls
done
